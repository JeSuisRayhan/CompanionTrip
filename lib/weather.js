...
export async function fetchDayWeather(day, dateISO, fallbackLocation) {
  ...
  // If the day's specific place name doesn't geocode (e.g. an attraction name
  // or a "District A / District B" combo), fall back to the trip's main
  // location before giving up entirely.
  if (!coords && fallbackLocation && fallbackLocation.trim() && fallbackLocation.trim() !== location) {
    coords = await geocodeLocation(fallbackLocation);
  }
