...
export async function addDocument(tripId, { title, category, tempUri }) {
  await ensureDocsDir();
  const docId = uid();
  ...
  return updateTrip(tripId, (trip) => ({
    ...
    documents: [
      ...(trip.documents || []),
      { id: docId, title: title.trim() || "Document", category: category || "autre", uri: destUri, addedAt: new Date().toISOString() },
    ],
  }));
}
